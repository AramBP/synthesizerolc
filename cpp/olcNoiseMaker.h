...
public:
	...
	bool Create(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		// Link headers naar block memory
...
private:
	...
	// basis thread. deze loop antwoord naar inputs van de soundcard en maakt de 'blocks'
	// met de audio data de inputs zijn beschikbaar tot het geluid        
