...
namespace synth
{
	...
	// maakt de frequantie naar een schuine snelheid
	...
	struct note
	{
		int id;		// positie in schaal
		FTYPE on;	// tijd noot is geactiveerd
		FTYPE off;	// tijd noot is gedeactiveerd
		bool active;
		...
		const FTYPE dLFOHertz = 0.0, const FTYPE dLFOAmplitude = 0.0, FTYPE dCustom = 50.0)
	{
		...
		switch (nType)
		{
		case OSC_SINE: // Sine wave tussen -1 and +1
			return sin(dFreq);

		case OSC_SQUARE: // Square wave tussen -1 and +1
			return sin(dFreq) > 0 ? 1.0 : -1.0;

		case OSC_TRIANGLE: // Triangle wave tussen -1 and +1
	...
	struct envelope_adsr : public envelope
	{
		...
		virtual FTYPE amplitude(const FTYPE dTime, const FTYPE dTimeOn, const FTYPE dTimeOff)
		{
			...
			if (dTimeOn > dTimeOff) // Note staat aan
			{
			...
			// Amplitude hoort niet negatief te zijn
			if (dAmplitude <= 0.01)
				dAmplitude = 0.0;
...
// Function word gebruikt bij olcNoiseMaker.h om geluid waves te maken
// Returns amplitude (-1.0 to +1.0) als een functe van tijd
...
int main()
{
	...
	wcout << "synthesizer, made by AramB" << endl
		<< "Meerdere FM Oscillators, Sequencing, Polyphony, bedankt OLC voor de duidelijke uitleg en de Noisemaker.h file erg koel" << endl << endl;
	...
	while (1)
	{
		...
		for (int k = 0; k < 16; k++) 
		{
			...
			{
				// noot is niet gevonden in sequencer
				if (nKeyState & 0x8000)
				{
					// noot is ingedrukt
					synth::note n;
